...
char s[K];
ll a[K];
ll f[2][40000][3]; // $f_{i,s,j}$ 表示匹配到第 $i$ 位，内部 LCS 自动机状态为 $s$，字符串的最后字符串分别为 $\text{otherwise}$、$\texttt{N}$、$\texttt{NO}$。 
ll nxt[3][3]; // 上几位是 $\text{otherwise}$、$\texttt{N}$、$\texttt{NO}$，这一位是 $\texttt{N}$、$\texttt{O}$、$\texttt{I}$，那么这一位的结尾是 $\text{otherwise}$、$\texttt{N}$、$\texttt{NO}$ 的哪一个？ 
ll ans[K];
...
int main() {
	...
	nxt[0][2] = 0;
	nxt[1][0] = 1;
	nxt[1][1] = 2; // 上几位是 $\texttt{N}$，这一位是 $\texttt{N}$，那么这一位的结尾是 $\texttt{N}$。上、下同。
	nxt[1][2] = 0;
	nxt[2][0] = 1;
	nxt[2][1] = 0;
	nxt[2][2] = 3;
	...
	for(ll i = 1; i <= n; i++) {
		...
		for(ll s = 0; s < (1<<k); s++) { // $i-1$ 的 LCS 自动机状态 
			unzip(s);
			for(ll j = 0; j <= 2; j++) { // $i-1$ 的结尾是 $\text{otherwise}$、$\texttt{N}$、$\texttt{NO}$。 
				if(!f[(i-1)%2][s][j]) continue; // 卡常 
				for(ll c = 0; c <= 2; c++) { // 枚举第 $i$ 位字母 $\texttt{N}$、$\texttt{O}$、$\texttt{I}$。 
					if(j == 2 && c == 2) continue;		// $\texttt{NO}$ 结尾不能加上 $\texttt{I}$
